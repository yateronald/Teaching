...
function buildBatchAssignmentTeacherTemplate({ teacherName, batchName, frenchLevel, startDate, endDate, schedules, logoCid, studentCount }) {
  const subject = `New batch assigned: ${batchName} — ${frenchLevel}`;
  const duration = computeDurationLabel(startDate, endDate);
  const bodyHtml = `
    <p style="margin:0 0 12px; font-size:16px;">Bonjour ${teacherName || 'Teacher'},</p>
    <p style="margin:0 0 16px; font-size:15px; color:#374151;">You have been assigned to a new batch. Here are the details:</p>
    ...
    <p style="margin:18px 0 0; font-size:13px; color:#6B7280;">If any detail is incorrect, please contact the administrator.</p>
  `;

  const html = baseHtml({ subject, headerTitle: 'New Batch Assigned', bodyHtml, logoCid });
  const text = `Bonjour ${teacherName || 'Teacher'},\n\nYou have been assigned to a new batch.\n\nBatch: ${batchName}\nLevel: ${frenchLevel}\nDuration: ${duration}\nStudents: ${typeof studentCount === 'number' ? studentCount : '-'}\n\nTimetable:\n${renderScheduleText(schedules)}\n\nIf any detail is incorrect, please contact the administrator.`;
  return { subject, html, text };
}

module.exports = { buildBatchAssignmentTeacherTemplate };